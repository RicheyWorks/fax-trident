...
> **Status (2026-05-19):** in progress. Layer 2 (repository slices) shipped in batch 4.2 — every custom query and derived finder now has a green test against a Flyway-migrated H2 schema. Layer 3 (controller slices) and the most-changed parts of Layer 4 (JWT security probes) shipped in batch 4.1. `mvn test` BUILD SUCCESS 53/53. Remaining: Layer 1 unit tests, Layer 5 Testcontainers integration, Layer 6 flaky-inbound rewrite, Layer 7 TestFX. The plan below is unchanged otherwise.
>
...
### What exists today (2026-05-19, post batch 4.2)
...
- **New (batch 4.2):** `UserRepositoryTest` (2 cases), `ContactRepositoryTest` (7 cases), `FaxLogRepositoryTest` (10 cases), `FaxMetadataRepositoryTest` (10 cases) — every custom `@Query` and derived finder now exercised against the Flyway-migrated schema.

Total: 53 tests across 10 classes. Coverage focus is the auth surface (most-changed by tech-debt #5), the request-shaping controllers, and the JPQL layer.
