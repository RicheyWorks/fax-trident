...
> **Status (2026-05-18):** in progress. Layer 3 (controller slices) and the most-changed parts of Layer 4 (JWT security probes) shipped in batch 4.1, `mvn test` BUILD SUCCESS 24/24 — see tracker. Remaining: Layer 1 unit tests, Layer 2 repository slices, Layer 5 Testcontainers integration, Layer 6 flaky-inbound rewrite, Layer 7 TestFX. The plan below is unchanged otherwise.
>
...
### What exists today (2026-05-18, post batch 4.1)
...
- **New (batch 4.1):** `AuthControllerTest` (4 cases), `AdminControllerTest` (3 cases), `FaxControllerTest` (6 cases), `JwtSecurityIntegrationTest` (5 cases).

Total: ~24 tests across 6 classes. Coverage focus is the auth surface (most-changed by tech-debt #5) and the request-shaping controllers.

Gaps (not yet covered):
- Repository slice tests for each `@Query`.
