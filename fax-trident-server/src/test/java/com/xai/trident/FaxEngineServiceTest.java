...
public class FaxEngineServiceTest {
    ...
    public void testSendFax() {
        ...
        // This matcher catches Redis sets whose value is a Long. After the 2.12 fix
        // (FaxController.send no longer pre-writes "Unknown"; sendFax owns the
        // get-or-create via findOrCreateContact), sendFax issues exactly two
        // Long-typed sets: contact.getId() at the start (":contactId") and
        // metadata.getId() once the PDDocument has loaded (":metadataId"). The
        // ":pages" set is an int/Integer and does not match anyLong(). String
        // sets (":status", ":number", ":barcodePath") also don't match.
        // If a third Long-valued set appears here, audit the change before
        // bumping this number — it likely reintroduces the duplication 2.12 closed.
