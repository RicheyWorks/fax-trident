...
public class RateLimitAspect {
    ...
    private String resolveKey(ProceedingJoinPoint joinPoint, String keyTemplate) {
        ...
        // Request-scoped variables: #request (the HttpServletRequest itself,
        // for callers that want raw access to headers/path/etc.) and
        // #ipAddress (a convenience for the common "rate-limit unauthenticated
        // endpoint per client IP" pattern).
        //
        // X-Forwarded-For trust: getRemoteAddr() returns the TCP peer address
        // by default, which is the proxy's address when the app sits behind
        // one. Operators behind a trusted proxy should set
        // `server.forward-headers-strategy: native` (or `framework`) so Spring
        // Boot rewrites getRemoteAddr() based on Forwarded / X-Forwarded-For.
        // The aspect deliberately does NOT parse the headers itself — doing
        // that without knowing where the trust boundary sits opens an IP-spoof
        // hole on naive deployments.
        HttpServletRequest request = currentRequestOrNull();
        ctx.setVariable("request", request);
        ctx.setVariable("ipAddress", request != null ? request.getRemoteAddr() : "unknown-ip");
    ...
    /**
     * Returns the current request, or {@code null} if the aspect somehow fires
     * outside a request scope (shouldn't happen for the controllers it's
     * applied to, but we guard so a misconfiguration doesn't NPE).
     */
