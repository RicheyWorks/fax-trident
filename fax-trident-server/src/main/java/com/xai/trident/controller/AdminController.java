...
public class AdminController {
    ...
    // No @Transactional — Redis-only, no JPA. (2.18)
    //
    // Endpoint URL retained as `/prediction-analytics` even though the
    // underlying service was renamed (SmartAssist → ContactSuggestion in
    // tech-debt #2 close-out). Renaming the URL would be a breaking change
    // for any external admin tooling. The internal SCAN pattern was updated
    // to the new `suggest:*` prefix to match where ContactSuggestionService
    // actually writes today. Existing `predict:*` keys from before the rename
    // are orphaned (1-hour TTL); they'll age out, or operators can SCAN+DEL
    // once to free memory immediately.
    public ResponseEntity<Map<String, Object>> getPredictionAnalytics(Authentication auth) {
        String username = auth.getName();
        logger.info("User '{}' fetching suggestion analytics...", username);
        Map<String, Object> analytics = new HashMap<>();
        long successfulSuggestions = 0;
        for (String key : scanKeys("suggest:*")) {
            Object raw = redisTemplate.opsForValue().get(key);
            // The previous version cast Object -> String unconditionally,
            // which NPE'd whenever the value had been evicted between
            // KEYS and GET. Treat null / non-string as a non-success. (Failure
            // sentinels are no longer cached as of the 2026-05-17 fix in
            // ContactSuggestionService, so a non-null cached value is always
            // a real suggestion — but the guard stays for defense-in-depth.)
            if (raw instanceof String) {
                successfulSuggestions++;
            }
        }
        // Response key kept as "successfulPredictions" for backwards-compat
        // with any admin tooling that already parses it; only the
        // implementation changed.
        analytics.put("successfulPredictions", successfulSuggestions);
        return ResponseEntity.ok(analytics);
    }
